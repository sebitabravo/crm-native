...
function agregarResultado() {
	...
	fetch('https://crm-react.sbravo.app/api/resultado', requestOptions)
		.then((response) => {
...
function completarFila(element, index, arr) {
	...
	arr[index] = document.querySelector('#tbl_resultado tbody').innerHTML += `<tr>
...
function obtenerDatosEliminacion(id_resultado) {
	...
	fetch(
		'https://crm-react.sbravo.app/api/resultado/' + id_resultado,
		requestOptions
	)
		.then((response) => response.json())
...
function completarEtiquetaEliminar(element) {
	...
	document.getElementById('lbl_eliminar').innerHTML =
		'¿Desea eliminar este resultado? <b>' + nombreResultado + '</b>';
}
...
function actualizarResultado() {
	...
	fetch(
		'https://crm-react.sbravo.app/api/resultado/' + g_id_resultado,
		requestOptions
	)
		.then((response) => {
			if (response.status == 200) {
