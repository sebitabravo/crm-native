...
function agregarUsuario() {
	...
	fetch('https://crm-react.sbravo.app/api/usuario', requestOptions)
		.then((response) => {
...
function completarFila(element, index, arr) {
	...
	arr[index] = document.querySelector('#tbl_usuario tbody').innerHTML += `<tr>
...
function obtenerDatosEliminacion(id_usuario) {
	...
	fetch(
		'https://crm-react.sbravo.app/api/usuario/' + id_usuario,
		requestOptions
	)
		.then((response) => response.json())
...
function actualizarUsuario() {
	...
	fetch(
		'https://crm-react.sbravo.app/api/usuario/' + g_id_usuario,
		requestOptions
	)
		.then((response) => {
			if (response.status == 200) {
