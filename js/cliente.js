...
function agregarCliente() {
	...
	fetch('https://crm-react.sbravo.app/api/cliente', requestOptions)
		.then((response) => {
...
function completarFila(element, index, arr) {
	...
	arr[index] = document.querySelector('#tbl_cliente tbody').innerHTML += `<tr>
...
function obtenerDatosEliminacion(id_cliente) {
	...
	fetch(
		'https://crm-react.sbravo.app/api/cliente/' + id_cliente,
		requestOptions
	)
		.then((response) => response.json())
...
function actualizarCliente() {
	...
	fetch(
		'https://crm-react.sbravo.app/api/cliente/' + g_id_cliente,
		requestOptions
	)
		.then((response) => {
			if (response.status == 200) {
